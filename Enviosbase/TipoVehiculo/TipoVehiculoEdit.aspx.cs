        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (obj.Id == 0)
                    {
                        ltScripts.Text = Business.Utils.MessageBox("Atención", "Hay un tipo de vehiculo creado con la misma información, por favor verifique la información e intente nuevamente", null, "error");
                        return;
                    }
                }
                ltScripts.Text = Business.Utils.MessageBox("Atención", "El registro ha sido guardado exitósamente", "TipoVehiculoList.aspx", "success");
            }
            catch (Exception)
            {
